@app.module 'CardEditorLayout', (CardEditorLayout) ->
	...
	class CardEditorLayout.CardEditorLayoutLayout extends Marionette.LayoutView
		logging: on
		counter: 0
		...
		_showGuiPanels: =>
			@layersPanelRegion.show new CardEditorLayout.views.LayersPanel 
				state: @editorState
				model: @model

			@textPanelRegion.show new CardEditorLayout.views.TextPanel 
				state: @editorState
				model: @model
			@listenTo @textPanelRegion.currentView, 'text:changed', @onTextChange

			@shapesPanelRegion.show new CardEditorLayout.views.LayerChildsPanel 
				state: @editorState
				model: @model

			@iconsPanelRegion.show new CardEditorLayout.views.IconsPanel 
				state: @editorState
				model: @model
			@listenTo @iconsPanelRegion.currentView, 'childview:click:icon', @onAddIcon

			# debugger;
			@backgroundsPanelRegion.show new CardEditorLayout.views.BackgroundsPanel
				state: @editorState
				model: @model
		...
		drawRandomBackgroundButton: =>
			...
			@stage.background = new app.Cards.CardModel()
			# app.shared.generators.gradientGen.draw(layer, @stage.background )
			@draw()
		...
		draw: =>
			...
			@stage.draw()
			layer = @stage.children[0]
			@stage.background = @stage.background or new app.Cards.CardModel()
			if layer and layer.canvas
				app.shared.generators.gradientGen.draw(layer.canvas._canvas, @stage.background )
