window.app.module 'Cards', (Cards) ->
	class Cards.CardsView extends Marionette.CompositeView
		logging: off
		counter: 0
		...
		template: (model) ->
			templatizer.cards.cardsGreed @model
		...
		initialize: ->
			...
			@childView = Cards.CardView
			
			@state      = new Backbone.Model()
			@model      = new Cards.stepForm.StepFormModel()
			@collection = new Cards.CardsCollection()
